...
use mclang::{BuildOptions, CheckOptions, build_file, check_file, serve};
        ...
        "build" => {
            ...
            while let Some(arg) = args.next() {
                match arg.as_str() {
                    ...
                    "--function-permission-level" => {
                        let value = args
                            .next()
                            .ok_or_else(|| "--function-permission-level 缺少等级参数".to_owned())?;
                        permission_level = Some(parse_permission_level(&value)?);
                    }
            ...
            let result = build_file(
                ...
                &BuildOptions {
                    description,
                    deny_raw,
                    function_permission_level: permission_level
                        .unwrap_or(mclang::DEFAULT_FUNCTION_PERMISSION_LEVEL),
                },
            )?;
            println!(
                "已生成 {} 个文件：{}",
...
fn parse_permission_level(value: &str) -> Result<u8, String> {
    match value.parse::<u8>() {
        Ok(level @ 2..=4) => Ok(level),
        _ => Err(format!(
            "`--function-permission-level` 只接受 2、3、4，实际为 `{value}`"
        )),
    }
}
    ...
    println!(
        "mclang — Minecraft 26.3 数据包编译器\n\n\
         用法:\n  \
            ...
            \t[--function-permission-level <2|3|4>]\n  \
            mclang check <源文件.mcl|项目目录> [--deny-raw] [--function-permission-level <2|3|4>]\n  \
            mclang lsp\n  \
            mclang help\n\n\
         默认输出目录为 build/<源文件名>。\n\
