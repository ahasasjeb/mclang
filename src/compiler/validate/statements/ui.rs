use super::*;
use crate::ast::{BossBarAction, BossBarProperty, ParticleCommand, PostEffectAction, TitleAction, UiCommand};
    ...
    match command {
        ...
        UiCommand::Dialog { targets, dialog } => {
            ...
            if let Some(dialog) = dialog {
                validate_id("dialog", "对话框", dialog, span, diagnostics);
            }
        }
        ...
        UiCommand::PostEffect(action) => match action {
            PostEffectAction::Add { targets, effect } | PostEffectAction::Remove { targets, effect } => {
                player_target(targets, false, span, ctx, diagnostics);
                validate_id("post_effect", "后处理效果", effect, span, diagnostics);
            }
            PostEffectAction::Clear(targets) => player_target(targets, false, span, ctx, diagnostics),
    ...
    match target {
        Holder::SelfEntity => {
            if !ctx.context.satisfies(ExecutionContext::Player) {
                diagnostics.push(Diagnostic::new("目标 self/自身 需要 @player 玩家执行上下文", span));
            }
        }
        Holder::Origin => diagnostics.push(Diagnostic::new("界面命令不支持 origin/投掷者 目标；请在 execute on origin 块中使用 self", span)),
        Holder::Query(name, query_span) => {
            if let Some(query) = require_player_query(name, *query_span, ctx, diagnostics)
                && single && query.limit != Some(1)
            {
                diagnostics.push(Diagnostic::new(
                    format!("目标查询 `{name}` 需要 limit(1) 的单个玩家"), *query_span,
                ));
            }
        }
    }
...
fn validate_bossbar(
    ...
    let id = match action {
        ...
        BossBarAction::Set { id, property } => {
            match property {
                BossBarProperty::Name(name) => crate::compiler::validate::components::validate_component(name, ctx, diagnostics),
                BossBarProperty::Color(color) => validate_enum("bossbar_color", "首领栏颜色", color, span, diagnostics),
                BossBarProperty::Style(style) => validate_enum("bossbar_style", "首领栏样式", style, span, diagnostics),
                BossBarProperty::Value(value) if *value > i32::MAX as u32 => diagnostics.push(Diagnostic::new("首领栏数值不能超过 2147483647", span)),
                BossBarProperty::Max(value) if *value == 0 || *value > i32::MAX as u32 => diagnostics.push(Diagnostic::new("首领栏最大值必须是 1 到 2147483647", span)),
                BossBarProperty::Players(Some(targets)) => player_target(targets, false, span, ctx, diagnostics),
                _ => {}
            }
            id
        }
    ...
    if !valid_resource_location(id) {
        diagnostics.push(Diagnostic::new(format!("`{id}` 不是有效的首领栏资源位置"), span));
    }
}
...
fn validate_particle(
    ...
    if let Some(speed) = &particle.speed
        && !speed.parse::<f64>().is_ok_and(|value| value >= 0.0 && value <= f32::MAX as f64)
    {
