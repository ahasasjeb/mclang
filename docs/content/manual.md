...
条件复用 `if`/`while` 的整套条件族（谓词、方块、生物群系、比较、`&&`/`||`/`!` 等），多个条件从左到右短路，全部成立才执行块体；`unless` 是条件的否定。条件在修饰符建立的上下文里求值，所以 `execute as(players) at(players) if block(pos(~, ~-1, ~), block_state("minecraft:stone"))` 检查的是每名玩家脚下的方块。修饰符产生多个执行来源时，每个来源依次完成条件求值与块体执行，先前来源的修改对后续来源可见。
...
运算符：一元 `-`，二元 `+`、`-`、`*`、`/`、`%`，括号分组；优先级与常见语言一致（`* / %` 高于 `+ -`）。纯常量运算会在编译期求值，除以零或结果超出 i32 范围会直接报错；含运行期计分值的运算仍沿用原版计分板的 32 位整数语义。
