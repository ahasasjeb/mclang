:::lede Mclang 是面向 Minecraft Java Edition 26.3 的数据包编程语言。用结构化语法描述数据包功能，编译器检查源码并生成 Minecraft 函数、资源文件与数据包目录。
:::
...
- **底层逃生口与严格模式**：`run`、字符串形式的 `execute` 与 `return run` 接受原生命令文本。`--deny-raw` 会拒绝这些写法，也会拒绝运行期 `with` 宏调用和声明 `nbt` 参数的宏函数。
...
| `mclang build <输入> [-o 路径]` | 通过全部检查后写出数据包，默认输出到 `build/<名字>` |
| `--zip` | 直接生成可分发 ZIP；默认输出到 `build/<名字>.zip`，与 `-o` 合用时后者是 ZIP 文件路径 |
| `--deny-raw` | 严格模式：检查或构建时拒绝 `run`、字符串形式的 `execute` / `return run`、运行期 `with` 宏调用，以及声明 `nbt` 参数的宏函数 |
...
目录项目可提供 `assets/pack.png`。需要自定义格式范围、`overlays`、`filter` 或 `features` 时，提供完整的 `assets/pack.mcmeta`；编译器会检查已知字段，`--description` 始终覆盖模板描述。Overlay 文件放在 `assets/overlays/<directory>/`，构建时按原相对路径复制到数据包根的 `<directory>/`。这三类资源也会进入 `--zip` 产物。
...
`if` 与 `while` 保持当前执行上下文，不引入新实体。简单条件可以直接生成原版 `execute if/unless`；需要保存求值结果时，条件会生成 0/1 的临时计分项，再用 `matches 1` 或 `matches 0` 选择分支。带 `else` 的条件只求值一次，执行真分支后不会重新判断条件。
...
`break` 与 `continue` 只允许出现在 `for`/`while` 里：`each`、`spawn` 的每个实体会单独执行，不能用它跳出。需要跳转的循环有一个状态计分项（1 = continue，2 = break）：循环体里设置的循环状态在其所在辅助函数返回后立即生效，最多跳出一层循环；嵌套循环独立处理自己的跳转，无跳转的外层循环不会因此分配状态计分项。
...
对查询结果逐个执行块内语句，并建立新的执行上下文：玩家查询进入玩家上下文，其他类型进入非玩家实体上下文。多实体查询保留辅助函数边界：先完整执行一个实体的块体，再对下一个实体求值块内条件，因此后一个实体能看到前一个实体对全局计分变量的修改。`limit(1)` 查询的安全单命令块可以直接内联。
...
`execute` 先按顺序调整执行上下文，再运行代码块；产物使用原版 `execute` 命令链，必要时进入辅助函数。子句顺序固定为**修饰符 → 条件 → store**：条件与 store 依赖修饰符建立的上下文，修饰符写在条件之后、重复的修饰符、条件写在 store 之后都会编译报错。
...
条件复用 `if`/`while` 的整套条件族（谓词、方块、生物群系、比较、`&&`/`||`/`!` 等），多个条件从左到右短路，全部成立才执行块体；`unless` 是条件的否定。条件在修饰符建立的上下文里求值，所以 `execute as(players) at(players) if block(pos(~, ~-1, ~), block_state("minecraft:stone"))` 检查的是每名玩家脚下的方块。修饰符产生多个执行来源时，按原版规则逐个条件筛选当前全部来源，再把剩余来源交给下一个条件；整个条件链完成后才执行块体。块体中的嵌套条件属于块体，各来源依次执行块体时才求值，不能提前并入外层条件链。
...
- 末尾的计分赋值保留可观察的赋值结果，包括 `counter = counter`、`counter = counter + 0` 等自赋值；不会因消除赋值而错误捕获前一条命令或右侧调用的结果。
...
严格模式检查失败：项目包含 4 条底层语句或不安全宏（run/execute/return run/运行期 with/nbt 宏片段）
```
...
运算符：一元 `-`，二元 `+`、`-`、`*`、`/`、`%`，括号分组；优先级与常见语言一致（`* / %` 高于 `+ -`）。纯常量运算会在编译期求值，除以零或结果超出 i32 范围会直接报错；含运行期计分值的运算仍沿用原版计分板的 32 位整数语义。常量与运行期的除法、取模都遵循原版 `floorDiv`/`floorMod`：商向负无穷取整，非零余数与除数同号，例如 `-3 / 2 == -2`、`-3 % 2 == 1`、`3 % -2 == -1`。
