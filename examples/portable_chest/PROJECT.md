...
2. 只丢出一个该绿宝石。数据包会在落点召唤一辆不可被普通伤害破坏的箱子矿车；绿宝石不会被消耗，`give(origin, self.item)` 会把它原样放回投掷者背包。
3. 打开矿车并放入任意物品。
...
触发用的绿宝石不会被消耗或复制：数据包先给掉落物实体打上 `portable_chest_processed` 标签，避免它每刻重复切换；随后 `give(origin, self.item)` 读取掉落物的物品 NBT，通过原版 `execute on origin` 找到投掷者，再用 `item replace` 把完整物品堆放进投掷者背包的第一个空槽（编译器为此生成一个 `filtered` 槽位来源资源）。复制成功后源槽被清空，掉落物自行消失；名称、附魔、Lore 等组件原样保留。投掷者不存在或背包已满时不修改源实体，绿宝石留在原处。
